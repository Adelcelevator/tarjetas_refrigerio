...
use crate::{models::{data_model::postgres::{detallle_pago_model::DetallePago, pago_model::{Pago, PagoReq}, persona_model::{BuscarPersonaCobro, PersonaCobroConsulta}}, responses::{response::{Response, ResponseData}, tipos_pagos_unidades_response::TiposPagosUnidadesResponse}}, repository::postgres::{detalle_pago_repository::guardar_detalle, pago_repository::{anular_pago, guardar_pago_repo}, persona_repository::buscar_persona_para_cobro, tarjeta_repository::cargar_saldo_tarjeta, tipo_pago_repository::get_tipos_pagos, unidades_repository::get_unidades}, utils::enums::{errors::service_error::ServiceError, estados_enum::Estados}};
...
pub async fn guardar_pago(con:&mut AsyncPgConnection,
    ...
    for dato in guardar.detalle_pagos.clone() {
        let Some(val) = Decimal::from_f32_retain(dato.detalle_pago_valor) else{
            return Err(ServiceError::ValidationError("No se puedo transformar los valores del detalle.".to_string()));
        };
        valor = valor + val;
    }

    let Some(cobrar) = Decimal::from_f32_retain(guardar.cobrar) else {
        return Err(ServiceError::ValidationError("No se puedo transformar el valor total a cobrar.".to_string()));
    };

    if valor > cobrar {
        return Err(ServiceError::ValidationError("El valor total del detalle es superior al valor a guardar.".to_string()));
    }

    if valor < cobrar {
        return Err(ServiceError::ValidationError("El valor total del detalle es inferior al valor a guardar.".to_string()));
    }

    let Some(valor_cobro_decimal) = Decimal::from_f32(guardar.cobrar) else {
        return Err(ServiceError::ValidationError("No se pudo transformar el valor a cobrar.".to_string()));
    };

    let pago = Pago{
        pago_id:None,
        per_id: Some(guardar.per_id.clone()),
        pago_valor_total: valor_cobro_decimal,
    ...
    let Some(guardado) = guardar_pago_repo(con, pago).await? else {
    ...
    for detalle in guardar.detalle_pagos {
        ...
        if detalle.tipo_pago_id == 2 {
            let saldo = cargar_saldo_tarjeta(con, &guardar.per_id).await?;
            if saldo.is_empty() {
                anular_pago( con, guardado).await?;
    ...
    let guardado_det = guardar_detalle(con, detalle_guardar).await?;
